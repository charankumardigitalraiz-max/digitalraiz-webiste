...
export default function FloatingSocials() {
  ...
  const socials = [
    {
      name: "WhatsApp",
      icon: <MessageCircle className="w-4.5 h-4.5 sm:w-5 sm:h-5" />,
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-4 left-4 sm:bottom-6 sm:left-6 z-30 bg-gradient-to-r from-rose-500 via-pink-500 to-purple-600 hover:from-rose-600 hover:via-pink-600 hover:to-purple-700 text-white font-mono text-[10px] font-bold uppercase tracking-wider px-3.5 sm:px-4.5 py-1.5 rounded-full shadow-lg border border-white/30 hover:scale-105 active:scale-95 transition-all duration-300 flex items-center gap-2 cursor-pointer group select-none"
        aria-label="Open Social Links"
      >
        {/* Animated Chevron Badge */}
        <div className="w-5 h-5 sm:w-5.5 sm:h-5.5 rounded-full bg-white/20 backdrop-blur-xs flex items-center justify-center text-white group-hover:scale-110 transition-transform">
          <ChevronUp className="w-3.5 h-3.5 animate-bounce" />
        </div>

        {/* Text */}
        <span className="tracking-wider text-white font-extrabold drop-shadow-xs">
          Connect
        </span>
  ...
  return (
    <div className="fixed bottom-4 left-4 sm:bottom-6 sm:left-6 z-30 flex items-center gap-1.5 sm:gap-2.5 p-1.5 sm:p-2 rounded-full bg-white/90 backdrop-blur-2xl border border-white/80 shadow-[0_15px_40px_rgba(0,0,0,0.15)] select-none pointer-events-auto transition-all duration-300 max-w-[calc(100vw-32px)] overflow-x-auto no-scrollbar">
      {socials.map((item) => (
        <a
          key={item.name}
          href={item.href}
          ...
          className={`group relative w-8.5 h-8.5 sm:w-9 sm:h-9 shrink-0 rounded-full ${item.bgColor} ${item.textColor} flex items-center justify-center shadow-md ${item.glow} hover:scale-110 active:scale-95 transition-all duration-300 border border-white/40`}
        >
          {item.icon}
          ...
          {item.name === "WhatsApp" && (
            <span className="absolute -top-0.5 -right-0.5 flex h-2.5 w-2.5 sm:h-3 sm:w-3">
              ...
              <span className="relative inline-flex rounded-full h-2.5 w-2.5 sm:h-3 sm:w-3 bg-emerald-400 border-2 border-white" />
            </span>
          )}

          {/* Hover Tooltip Ribbon (Appears ABOVE the button) */}
          <div className="absolute bottom-12 sm:bottom-14 left-1/2 -translate-x-1/2 px-2.5 py-1 rounded-xl bg-slate-900/95 backdrop-blur-md border border-white/15 text-white text-[9px] sm:text-[10px] font-mono font-bold uppercase tracking-wider whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 translate-y-2 transition-all duration-200 shadow-2xl flex items-center gap-1.5">
            <span>{item.name}</span>
            {item.badge && (
              <span className="px-1 py-0.5 rounded-md bg-emerald-500/30 text-emerald-300 text-[8px]">
                {item.badge}
              </span>
            )}
          </div>
      ...
      <button
        ...
        className="w-8.5 h-8.5 sm:w-9 sm:h-9 shrink-0 rounded-full bg-slate-100/90 hover:bg-slate-200 text-slate-500 hover:text-slate-800 flex items-center justify-center transition-all duration-200 border border-slate-200/60 ml-0.5 cursor-pointer group"
