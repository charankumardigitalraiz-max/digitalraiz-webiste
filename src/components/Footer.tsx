        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-8 mb-12">
          ...
          <div className="lg:col-span-4 space-y-5">
            ...
            <div className="pt-3 flex items-center gap-2.5">
              <a href={officeDetails.socialLinks?.find(s => s.name === "Facebook")?.url || "https://www.facebook.com/digitalraiz/"} target="_blank" rel="noopener noreferrer" className="w-9.5 h-9.5 rounded-full bg-[#1877f2] border border-[#1877f2] text-white flex items-center justify-center transition-all duration-300 hover:scale-115 hover:shadow-md shadow-xs" aria-label="Facebook">
                <svg className="w-4.5 h-4.5 fill-current" viewBox="0 0 24 24"><path d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.84 3.44 8.87 8 9.8V15H8v-3h2V9.5C10 7.57 11.57 6 13.5 6H16v3h-2c-.55 0-1 .45-1 1v2h3v3h-3v6.95c5.05-.5 9-4.76 9-9.95z" /></svg>
              </a>
              <a href={officeDetails.socialLinks?.find(s => s.name === "Twitter")?.url || "https://twitter.com/digital_raiz"} target="_blank" rel="noopener noreferrer" className="w-9.5 h-9.5 rounded-full bg-slate-900 border border-slate-900 text-white flex items-center justify-center transition-all duration-300 hover:scale-115 hover:shadow-md shadow-xs" aria-label="Twitter">
              ...
              <a href={officeDetails.socialLinks?.find(s => s.name === "YouTube")?.url || "https://www.youtube.com/@digitalraiz2023/featured"} target="_blank" rel="noopener noreferrer" className="w-9.5 h-9.5 rounded-full bg-[#ff0000] border border-[#ff0000] text-white flex items-center justify-center transition-all duration-300 hover:scale-115 hover:shadow-md shadow-xs" aria-label="Youtube">
              ...
              <a href={officeDetails.socialLinks?.find(s => s.name === "LinkedIn")?.url || "https://www.linkedin.com/company/digital-raiz/"} target="_blank" rel="noopener noreferrer" className="w-9.5 h-9.5 rounded-full bg-[#0a66c2] border border-[#0a66c2] text-white flex items-center justify-center transition-all duration-300 hover:scale-115 hover:shadow-md shadow-xs" aria-label="LinkedIn">
              ...
              <a href={officeDetails.socialLinks?.find(s => s.name === "Pinterest")?.url || "https://in.pinterest.com/digitalraiz/"} target="_blank" rel="noopener noreferrer" className="w-9.5 h-9.5 rounded-full bg-[#e60023] border border-[#e60023] text-white flex items-center justify-center transition-all duration-300 hover:scale-115 hover:shadow-md shadow-xs" aria-label="Pinterest">
                <svg className="w-4.5 h-4.5 fill-current" viewBox="0 0 24 24"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.08 3.16 9.4 7.63 11.16-.1-.94-.2-2.39.04-3.41.22-.93 1.4-5.96 1.4-5.96s-.36-.72-.36-1.77c0-1.66.96-2.9 2.17-2.9 1.02 0 1.51.77 1.51 1.69 0 1.03-.66 2.56-.99 3.98-.28 1.19.6 2.16 1.77 2.16 2.13 0 3.76-2.25 3.76-5.49 0-2.87-2.06-4.88-5.01-4.88-3.41 0-5.42 2.56-5.42 5.21 0 1.03.4 2.14.89 2.74.1.12.11.23.08.35-.09.38-.3 1.22-.34 1.39-.06.23-.19.28-.43.17-1.59-.74-2.59-3.07-2.59-4.94 0-4.02 2.92-7.71 8.42-7.71 4.42 0 7.86 3.15 7.86 7.37 0 4.39-2.77 7.93-6.61 7.93-1.29 0-2.5-0.67-2.91-1.46L9.89 20c-.39 1.48-1.44 3.32-2.15 4.46C8.82 23.82 10.34 24 12 24c6.63 0 12-5.37 12-12S18.63 0 12 0z" /></svg>
              </a>
              <a href={officeDetails.socialLinks?.find(s => s.name === "Instagram")?.url || "https://www.instagram.com/digital_raiz/"} target="_blank" rel="noopener noreferrer" className="w-9.5 h-9.5 rounded-full bg-gradient-to-tr from-amber-500 via-rose-500 to-purple-600 border border-rose-500 text-white flex items-center justify-center transition-all duration-300 hover:scale-115 hover:shadow-md shadow-xs" aria-label="Instagram">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <ul className="space-y-3.5 text-slate-600 font-light">
              <li className="flex gap-2.5 items-start group">
                <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-pink-500 to-rose-600 text-white flex items-center justify-center shrink-0 shadow-xs group-hover:scale-110 transition-transform mt-0.5">
                  <MapPin className="w-3.5 h-3.5" />
                </div>
                <span className="leading-relaxed font-medium text-sm">
                  {officeDetails.addressLines[0]}<br />
                  {officeDetails.addressLines[1]}
                </span>
              </li>
              <li className="flex gap-3.5 items-center group">
                <div className="w-8.5 h-8.5 rounded-xl bg-gradient-to-br from-violet-600 to-indigo-600 text-white flex items-center justify-center shrink-0 shadow-xs group-hover:scale-110 transition-transform">
                  <Phone className="w-4.5 h-4.5" />
                </div>
                <div className="flex flex-col gap-0.5">
                  {officeDetails.phones.map((phone, idx) => (
                    <a key={idx} href={`tel:${phone.replace(/[^0-9+]/g, "")}`} className="hover:text-pink-600 transition-colors font-semibold text-slate-700 text-sm">
                      {phone}
                    </a>
                  ))}
                </div>
              ...
              <li className="flex gap-3.5 items-center group">
                <div className="w-8.5 h-8.5 rounded-xl bg-gradient-to-br from-indigo-600 to-blue-600 text-white flex items-center justify-center shrink-0 shadow-xs group-hover:scale-110 transition-transform">
                  <Mail className="w-4.5 h-4.5" />
                </div>
                <div className="flex flex-col gap-0.5">
                  {officeDetails.emails.map((email, idx) => (
                    <a key={idx} href={`mailto:${email}`} className="hover:text-pink-600 transition-colors font-semibold text-slate-700 text-sm">
                      {email}
                    </a>
                  ))}
                </div>
